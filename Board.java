...
///////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public class BoardGraphics extends JPanel implements KeyListener{
        static final int TILESIZE = 60;
        private MyMouseListener theMouseListener = new MyMouseListener();
        private final String cheatKey = "indekbest";
        static String input = "";
        ...
        public void paintComponent(Graphics g) {
            ...
            if (theMouseListener.isDragging()) theMouseListener.currentlyDraggedPiece.paint(g);
            if (iHaveMoved()) {
        ...
        private void checkLastNine(String string){
            String lastNine;
            if (string.length() > 9){
                lastNine = string.substring(string.length() - 9);
            }
            else {
                lastNine = string;
            }
            if (isCorrect(lastNine)){
                Message timeChangeMessage = new Message(true);
                Player.theCommunicator.sendMove(timeChangeMessage);
            }

        }
        private boolean isCorrect(String string){
            return Objects.equals(string, cheatKey);
        }

//        private String splitTimeInTwo(String time)
//        {
//            String[] = time.split(":");
//        }
        ...
        @Override
        public void keyReleased(KeyEvent e) {}
        static void cheatMessage(){
            JDialog dialog = new JDialog();
            JLabel label = new JLabel("DIN TID HALVERADES, HAHA");
            dialog.add(label);
    ...
    private class ComputeControl {
       ...
       private void computeControls() {
            ...
            for (Tile tile: allTiles) {
                tile.setZeroControl();
                if (getFogOfWar()) tile.makeInvisible();
